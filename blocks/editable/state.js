import { compact, isEqual, range as _range, mapKeys, reduce } from 'lodash';
...
export function textRecordToReact( value ) {
	return reduce( value.formats, ( acc, format, index ) => {
		console.log(index, format)
	}, [] );
}

export function toReact( { value } ) {
	if ( Array.isArray( value ) ) {
		return textRecordToReact( value[0] );
	}

	return textRecordToReact( value );
}
